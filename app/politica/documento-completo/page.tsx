// Texto integral da Política (com TOC lateral). Acessado via "Documento completo" no hub.
...
export const metadata = { title: "Política — Documento completo" };

export default async function DocumentoCompletoPage() {
  const session = await auth();
  if (!session?.user) redirect("/login");

  const toc = extractToc(POLITICA_MD, 1);
  ...
  return (
    <main className="mx-auto max-w-[1400px] px-4 sm:px-6 py-6 space-y-5">
      <PageHeader
        ...
        title="Política DSF — texto integral"
        description="Versão consolidada (cláusulas e anexos). Para navegação por tema, volte ao hub."
        meta={
          <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
        ...
        <article className="min-w-0">
          <Card className="px-6 py-6 lg:px-10 lg:py-8">
            <MarkdownContent md={POLITICA_MD} />
          </Card>
        </article>
      </div>
    </main>
